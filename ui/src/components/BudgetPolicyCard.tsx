...
export function BudgetPolicyCard({
  ...
  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Gasto observado</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">{formatCents(summary.observedAmount)}</div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? `${summary.utilizationPercent}% do limite` : "Sem limite configurado"}
        </div>
      </div>
      <div>
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          Alerta leve em {summary.warnPercent}%{summary.paused && summary.pauseReason ? ` · pausado por ${summary.pauseReason}` : ""}
        </div>
      </div>
    </div>
  ) : (
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? "A execução está pausada para este projeto até que o orçamento seja aumentado ou o incidente seja dispensado."
          : "Heartbeats deste escopo estão pausados até que o orçamento seja aumentado ou o incidente seja dispensado."}
      </div>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          ...
          >
            ...
            {summary.paused ? "Pausado" : summary.status === "warning" ? "Aviso" : summary.status === "hard_stop" ? "Bloqueado" : "Saudável"}
          </div>
        </div>
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">Digite um valor em dólar não negativo válido.</p>
        ) : null}
      </div>
    );
  }
